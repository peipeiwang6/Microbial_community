...
F = {}
ncol = len(feature[0].split())
Location = {}
for inl in feature[1:]:
	tem = inl.strip().split()
	if tem[3] == 'Fert':
		F[tem[0]] = '1\t' + '\t'.join(tem[6:(ncol-3)])
	if tem[3] == 'Unfert':
		F[tem[0]] = '0\t' + '\t'.join(tem[6:86])
	Location[tem[0]] = [tem[2],tem[3],tem[ncol-3],tem[ncol-2],tem[ncol-1]]
...
title = 'sampleID\tY\tFertstat\t'+ '\t'.join(feature[0].split()[6:(ncol-3)])
out.write(title + '\n')
for id in D:
		out.write('%s\t%s\t%s\n'%(id,D[id][0],F[id]))
